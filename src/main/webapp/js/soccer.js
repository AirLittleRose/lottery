...
function to_xiazhu(){
	...
	var info=new xiazhu_info(game_id, game_time, home_name, away_name, sp, odds, num, type, pel, amount);
		...
		success:function(data){
			$("#info_tbody").html("");
			var str = "";
			for(var i=0; i<data.obj.game_id.length; i++){
				str += "<tr data-id='" + data.obj.game_id[i] + "' data-i='" + i + "' data-rs=''>  ";
				str += "<td>" + data.obj.game_id[i] + "</td> <td>" + data.obj.game_time[i] + "</td>";
				str += "<td>" + data.obj.home_name[i] + "&nbsp;&nbsp;VS&nbsp;&nbsp;" + data.obj.away_name[i] + "</td><td>胜平负</td>";
				str += "<td><span class='f-gray'>—</span></td><td class='betItemTd'><a href='javascript:;' class='betItem ' data-i='0'>";
				str += turnSp(data.obj.sp[i]) + "" + data.obj.odds[i] + "<i class='icon icon_gold'></i></a></td></tr>";
			}
			$("#info_tbody").html(str);
			$(".matchNum").text(num);
			$(".betTimes").text(pel);
